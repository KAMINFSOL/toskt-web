...
<body class="min-h-screen bg-[url('/public/images/wave.svg')] bg-no-repeat bg-bottom">
    <header class="flex justify-between bg-white/80 backdrop-blur-xl mt-4 sm:mt-2 p-3 sm:p-5 mx-auto rounded-xl shadow-xl max-w-370">
        <div class="flex gap-3 items-center">
            ...
            <h1 class="font-semibold text-xl font-serif">Реестр заявок - Профиль</h1>
        </div>
    ...
    <main>
        <div class="flex flex-col items-center gap-5 bg-white/80 backdrop-blur-xl mt-4 sm:mt-6 p-6 sm:p-10 mx-auto rounded-xl shadow-xl max-w-200">
            <div class="flex gap-5">
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex gap-3">
                        ...
                        <span>{{ Carbon\Carbon::parse(Auth::user()->birth_date)->format('d.m.Y') }}</span>
                    </div>
                    <span>{{ Auth::user()->email }}</span>
                    <span>{{ Auth::user()->post }}</span>
                    <span>{{ Auth::user()->subdivision }}</span>
                </div>
            </div>
            <div class="flex gap-2">
                ...
                <a href="{{ route('accounting_journal') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 text-blue-400">
